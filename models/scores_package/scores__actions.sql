    ...
    {% set max_block_date_query %}
        ...
        {% if execute %}
            ...
            {% if block_dates|length > 0 %}
                {{ log("==========================================", info=True) }}
                {{ log("Loading action data for blockchain: " ~ vars.GLOBAL_PROD_DB_NAME, info=True) }}
                {{ log("For block dates: " ~ block_dates|join(', '), info=True) }}
                {{ log("==========================================", info=True) }}
            {% else %}
                {{ log("==========================================", info=True) }}
                {{ log("No new action data to process for blockchain: " ~ vars.GLOBAL_PROD_DB_NAME, info=True) }}
                {{ log("==========================================", info=True) }}
            {% endif %}
        {% else %}
            {% set block_dates = [] %}
...
{% if (is_incremental() and block_dates|length > 0) or (not is_incremental()) %}
    ...
    eligible_events AS (
        ...
        LEFT JOIN {{ ref('scores__wrapped_assets') }} w ON e.contract_address = w.wrapped_asset_address AND w.blockchain = '{{ vars.GLOBAL_PROD_DB_NAME }}'

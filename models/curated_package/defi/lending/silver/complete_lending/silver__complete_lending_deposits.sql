...
aave_ethereum AS (
    SELECT
        ...
        A.event_name
    FROM
        {{ ref('silver__aave_ethereum_deposits') }} A
    WHERE
        token_symbol IS NOT NULL
...
fraxlend AS (
    SELECT
        ...
        A.event_name
    FROM
        {{ ref('silver__fraxlend_deposits') }} A
    WHERE
        token_symbol IS NOT NULL
...
comp_v2_fork AS (
    SELECT
        ...
        A.event_name
    FROM
        {{ ref('silver__comp_v2_deposits') }} A
    WHERE
        token_symbol IS NOT NULL
...
compound_v3 AS (
    SELECT
        ...
        A.event_name
    FROM
        {{ ref('silver__comp_v3_deposits') }} A
    WHERE
        token_symbol IS NOT NULL

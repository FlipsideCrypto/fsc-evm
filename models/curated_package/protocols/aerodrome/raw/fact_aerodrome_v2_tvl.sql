...
, sparse_balances AS (
    ...
    FROM
        ...
        LEFT JOIN {{ ref('price__ez_asset_metadata') }} t on t.token_address = b.contract_address
    WHERE 1=1
...
, full_table as (
    ...
    FROM
        full_balances fb
        LEFT JOIN {{ ref('price__ez_prices_hourly') }} p ON 
                (
                    p.hour = fb.date
                    AND fb.contract_address = p.token_address
                )
        -- left join native token price
        LEFT JOIN {{ ref('price__ez_prices_hourly') }} native_token ON
                (

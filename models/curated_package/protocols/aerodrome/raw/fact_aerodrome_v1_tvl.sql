...
,all_data AS (
    ...
    LEFT JOIN {{ ref('price__ez_prices_hourly') }} t0 ON r.date = t0.hour AND p.token0 = t0.token_address
    LEFT JOIN {{ ref('price__ez_prices_hourly') }} t1 ON r.date = t1.hour AND p.token1 = t1.token_address
    WHERE r.rn = 1

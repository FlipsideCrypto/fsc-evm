{{ config(
    materialized = 'incremental',
    incremental_strategy = 'delete+insert',
    unique_key = ['tx_hash', 'event_index'],
    cluster_by = ['block_timestamp::date'],
    tags = ['silver_protocols', 'balancer', 'v2', 'ethereum', 'pool_balance_changed', 'curated']
) }}

{# Get Variables #}
{% set vars = return_vars() %}

{# Log configuration details #}
{{ log_model_details() }}

{#
    Balancer V2 Ethereum Pool Balance Changed Events

    Tracks PoolBalanceChanged events for liquidity additions/removals.
    Note: Requires get_balancer_v2_PoolBalanceChanged macro implementation.
#}

{{ get_balancer_v2_PoolBalanceChanged(
    'ethereum',
    is_incremental(),
    vars.CURATED_LOOKBACK_HOURS,
    vars.CURATED_LOOKBACK_DAYS
) }}

...
WITH SWAP_DETAILS AS (
    ...
    LEFT JOIN
        {{ ref('price__ez_prices_hourly') }} t2 --{{ ref('price__ez_prices_hourly') }} t2
        on (lower(swaps.tokenIn) = lower(t2.token_address) and t2.hour = swaps.hour)
    LEFT JOIN
        {{ ref('price__ez_prices_hourly') }} t3 --{{ ref('price__ez_prices_hourly') }} t3
...
SWAPS_USD_RAW AS (
    ...
    LEFT JOIN {{ ref('core__ez_decoded_traces') }} source_fees --{{ ref('core__ez_decoded_traces') }} source_fees 

...
product_id_join AS (
    SELECT
        ...
        p.symbol,
        CASE
            {% for symbol, address in vars.CURATED_NADO_TOKEN_MAPPING.items() %}
    ...
    FROM
        logs_pull l
        LEFT JOIN {{ ref('silver__nado_dim_products') }}
        p
...
SELECT
    *,
    {{ dbt_utils.generate_surrogate_key(['tx_hash','event_index']) }} AS nado_collateral_id,

...
{{ config (
    ...
    tags = ['gold','balances','phase_4','heal','test_state']
) }}
...
WITH state_storage_realtime AS (
    ...
    SELECT
        ...
        post_storage_hex
    FROM
        {{ ref('silver__state_tracer_test_erc20') }}
        t
        INNER JOIN {{ ref('silver__balance_slots_test') }}
...
new_contracts AS (
    ...
    FROM {{ ref('silver__balance_slots_test') }}

...
{{ config (
    ...
    unique_key = "balances_erc20_daily_history_snapshot_records_id",
    cluster_by = "ROUND(block_number, -3)",
    post_hook = '{{ unverify_balances() }}',
    full_refresh = vars.GLOBAL_STREAMLINE_FR_ENABLED,
    tags = ['streamline','balances','history_snapshot','erc20','heal','phase_4']
) }}
--intended to run on an on-going basis, only for newly verified contracts
...
logs AS (
    ...
    {% if is_incremental() and var('HEAL_MODEL',false) %}
        ...
        ) --select only newly verified contracts for incremental
    {% else %}
        AND contract_address IN (
            SELECT
                token_address
            FROM
                verified_contracts
        ) --select all verified contracts for full-refresh / backfill
    {% endif %}
),
transfers AS (
    SELECT
...
SELECT
    ...
    address,
    contract_address,
    {{ dbt_utils.generate_surrogate_key(['block_date', 'address', 'contract_address']) }} AS balances_erc20_daily_history_snapshot_records_id,
    SYSDATE() AS inserted_timestamp,
    SYSDATE() AS modified_timestamp,
    '{{ invocation_id }}' AS _invocation_id
FROM
    to_do qualify (ROW_NUMBER() over (PARTITION BY balances_erc20_daily_history_snapshot_records_id
ORDER BY

...
{{ config (
    ...
    unique_key = "balances_native_daily_history_snapshot_records_id",
    cluster_by = "ROUND(block_number, -3)",
    full_refresh = vars.GLOBAL_STREAMLINE_FR_ENABLED,
    tags = ['streamline','balances','history_snapshot','native','phase_4']
) }}
--intended to run only once per chain, at the start of the balances backfill process
...
SELECT
    ...
    block_date,
    address,
    {{ dbt_utils.generate_surrogate_key(['block_date', 'address']) }} AS balances_native_daily_history_snapshot_records_id,
    SYSDATE() AS inserted_timestamp,
    SYSDATE() AS modified_timestamp,
    _inserted_timestamp,
    '{{ invocation_id }}' AS _invocation_id
FROM
    to_do qualify (ROW_NUMBER() over (PARTITION BY balances_native_daily_history_snapshot_records_id
ORDER BY

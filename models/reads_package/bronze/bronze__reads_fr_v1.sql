...
{% set partition_function = "TO_DATE(concat_ws('-', SPLIT_PART(file_name, '/', 3), SPLIT_PART(file_name, '/', 4), SPLIT_PART(file_name, '/', 5)))" %}
{% set partition_join_key = '_partition_by_modified_date' %}
...
{{ config (
    materialized = 'view',
    tags = ['bronze_reads','bronze_streamline_v1']
) }}

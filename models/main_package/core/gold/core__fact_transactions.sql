...
{{ config (
    ...
    -- post_hook = "ALTER TABLE {{ this }} ADD SEARCH OPTIMIZATION ON EQUALITY(block_number,tx_hash,from_address,to_address,origin_function_signature), SUBSTRING(input_data)", -- Moved to daily_search_optimization maintenance model

    ...
    new_transactions AS (
        SELECT
            ...
            {% if rpc_vars.blobGasPrice %}
                utils.udf_hex_to_int(
                    r.receipts_json :blobGasPrice :: STRING
                ) / pow(
                    10,
                    9
                ) AS blob_gas_price,
            ...
            {% if rpc_vars.blobGasUsed %}
                utils.udf_hex_to_int(
                    r.receipts_json :blobGasUsed :: STRING
                ) AS blob_gas_used,
            {% endif %}
            ...
            {% if rpc_vars.tokenRatio %}
                TRY_TO_NUMBER(
                    utils.udf_hex_to_int(
                        r.receipts_json :tokenRatio :: STRING
                    )
                ) AS token_ratio,
            {% endif %}
    ...
    SELECT
        ...
        {% if rpc_vars.blobGasPrice %}
            utils.udf_hex_to_int(
                ...
                10,
                9
            ) AS blob_gas_price_heal,
        {% endif %}

        {% if rpc_vars.blobGasUsed %}
            utils.udf_hex_to_int(
                r.receipts_json :blobGasUsed :: STRING
            ) AS blob_gas_used_heal,
        {% endif %}
        ...
        {% if rpc_vars.l1FeeScalar %}
            utils.udf_decimal_adjust(
                ((t.gas_price * pow(10, 9)) * utils.udf_hex_to_int(r.receipts_json :gasUsed :: STRING) :: bigint) + FLOOR(
            ...
            utils.udf_decimal_adjust(
                ((t.gas_price * pow(10, 9)) * utils.udf_hex_to_int(r.receipts_json :gasUsed :: STRING) :: bigint) + l1_fee_precise_raw_heal :: bigint,
                18
            ) AS tx_fee_precise_heal,
            {% elif vars.global_project_name == 'arbitrum' %}
            utils.udf_decimal_adjust(

...
WHERE
    ...
    transactions_fields AS (
        SELECT
            ...
            {% if tx_settings.uses_source_hash %}
    ...
    new_transactions AS (
        SELECT
            ...
            {% endif %}
            {% if tx_settings.uses_l1_columns %}
            ...
            txs.access_list,
            {% endif %}
            {% if tx_settings.uses_blob_base_fee %}
            ...
            {% endif %}
            {% if tx_settings.uses_l1_tx_fee_calc %}
            utils.udf_decimal_adjust(
                (

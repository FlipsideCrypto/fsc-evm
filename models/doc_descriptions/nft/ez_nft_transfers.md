...
- **Join with ez_nft_sales**: Use `tx_hash` and `event_index` to match with sales
- **Join with dim_nft_metadata**: Use `nft_address` and `tokenId` for metadata
- **Join with fact_transactions**: Use `tx_hash` for transaction context
- **Join with dim_contracts**: Use `nft_address` for collection details
...
SELECT 
    ...
    SUM(erc1155_value) AS total_erc1155_quantity
...
SELECT 
    ...
    COUNT(DISTINCT tokenId) AS unique_tokens_minted,
    MODE(CASE WHEN erc1155_value > 0 THEN erc1155_value ELSE 1 END) AS typical_mint_quantity
FROM <blockchain_name>.nft.ez_nft_transfers
WHERE is_mint = TRUE
...
WITH wallet_activity AS (
    SELECT 
        ...
        COUNT(DISTINCT nft_address) AS collections_interacted
    FROM (
        SELECT to_address AS address, COUNT(*) AS received, 0 AS sent, nft_address
        FROM <blockchain_name>.nft.ez_nft_transfers
        WHERE block_timestamp >= CURRENT_DATE - 30
        ...
        SELECT from_address AS address, 0 AS received, COUNT(*) AS sent, nft_address
        FROM ez_nft_transfers
        WHERE block_timestamp >= CURRENT_DATE - 30
...
**ERC-1155 Batch Transfer Analysis**
```sql
SELECT 
    tx_hash,
    event_index,
    nft_address,
    project_name,
    COUNT(*) AS tokens_in_batch,
    SUM(erc1155_value) AS total_quantity,
    ARRAY_AGG(DISTINCT tokenId) AS token_ids
FROM <blockchain_name>.nft.ez_nft_transfers
WHERE block_timestamp >= CURRENT_DATE - 7
    AND event_type = 'erc1155_TransferBatch'
GROUP BY 1, 2, 3, 4
HAVING COUNT(*) > 1
ORDER BY 5 DESC;
```
...
- **Token Standards**: Check `event_type` to distinguish between ERC-721 and ERC-1155
- **Quantity**: ERC-721 always has quantity 1, ERC-1155 uses `erc1155_value`
- **Mint/Burn Detection**: Use `is_mint` flag or check from/to addresses
- **Batch Transfers**: Multiple rows with same tx_hash and event_index for ERC-1155 batches
...
SELECT 
    ...
    project_name,
    COUNT(*) AS burns,
    COUNT(DISTINCT tokenId) AS unique_tokens_burned
FROM <blockchain_name>.nft.ez_nft_transfers
...
**Common Patterns**:
```sql
-- Find sequential vs random token_id minting
SELECT 
    nft_address,
    CASE 
        WHEN MAX(CAST(token_id AS NUMERIC)) - MIN(CAST(token_id AS NUMERIC)) + 1 = COUNT(DISTINCT token_id)
        THEN 'Sequential'
        ELSE 'Random/Sparse'
    END AS minting_pattern
FROM <blockchain_name>.nft.ez_nft_transfers
WHERE is_mint = TRUE
    AND TRY_CAST(token_id AS NUMERIC) IS NOT NULL
GROUP BY 1;
```
...
**Usage**:
- 0 for single transfers (ERC-721)
- 0,1,2... for batch transfers (ERC-1155)
- Maintains order within batch operations

**Batch Analysis**:
```sql
-- Analyze batch transfer sizes
SELECT 
    event_type,
    MAX(intra_event_index) + 1 AS batch_size,
...
The number of NFTs transferred for this specific tokenId.

**Standards**:
- ERC-721: Always 1 (non-fungible)
- ERC-1155: Can be 1 or more (semi-fungible)

**Note**: For ERC-1155, see `erc1155_value` for the specific quantity

{% enddocs %}

{% docs ez_nft_transfers_transfer_type %}

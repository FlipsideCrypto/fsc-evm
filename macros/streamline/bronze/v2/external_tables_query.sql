...
{% set partition_function = var((model ~ '_' ~ model_type ~ '_partition_function').upper(), 
    'CAST(SPLIT_PART(SPLIT_PART(file_name, '/', 4), '_', 1) AS INTEGER)') %}
{% set balances = var((model ~ '_' ~ model_type ~ '_balances').upper(), false) %}
{% set block_number = var((model ~ '_' ~ model_type ~ '_block_number').upper(), true) %}

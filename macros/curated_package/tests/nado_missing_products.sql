...
with recent_records as (
    ...
    where 
        ...
        {% if model.name == 'nado_market_stats' %}
...
invalid_product_ids as (
    ...
    from {{ ref('silver__nado_dim_products') }}
    where product_id not in (select product_id from recent_records)
    AND block_timestamp < sysdate() - INTERVAL '2 days'
    {% if filter %}
        AND {{ filter }}
    {% endif %}
    {% if model.name == 'nado_market_stats' %} --stops from test erroring right after deployment
    AND product_id in (
        select product_id
        from (
            select product_id,
...
{% test nado_product_level_recency(
    model,
    filter) %}

{% macro run_balances_erc20_history() %}
  ...
  {%- set params = {
      ...
      "sql_limit": vars.BALANCES_SL_HISTORY_BATCH_SIZE,
      ...
      "batch_size": vars.BALANCES_SL_HISTORY_BATCH_SIZE
  } -%}
...
{% macro run_balances_native_history() %}
  ...
  {% set find_batches_query %}
    SELECT 
      DISTINCT batch_id
    FROM {{ ref('streamline__balances_native_daily_history_to_do') }}
    ORDER BY batch_id ASC
  {% endset %}
  ...
  {% if execute %}
    ...
    {% for batch in batches %}
      {% set view_name = 'balances_native_daily_history_' ~ batch %}

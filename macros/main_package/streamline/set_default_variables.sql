...
{%- set node_secret_path = var('GLOBAL_NODE_VAULT_PATH', '') -%}
{%- set model_quantum_state = var((package_name ~ '_SL_' ~ model_name ~ '_' ~ model_type ~ '_quantum_state').upper(), 'streamline') -%}
{%- set testing_limit = var((package_name ~ '_SL_' ~ model_name ~ '_' ~ model_type ~ '_testing_limit').upper(), none) -%}
{%- set new_build = var((package_name ~ '_SL_' ~ model_name ~ '_' ~ model_type ~ '_new_build_enabled').upper(), false) -%}
...
{%- set order_by_clause = var((package_name ~ '_SL_' ~ model_name ~ '_' ~ model_type ~ '_order_by_clause').upper(), default_order) -%}

...
{%- set params = {
    ...
    "sql_limit": var((model_name ~ '_' ~ model_type ~ '_sql_limit').upper(), 2 * var('GLOBAL_BLOCKS_PER_HOUR', none) * multiplier),
    "producer_batch_size": var((model_name ~ '_' ~ model_type ~ '_producer_batch_size').upper(), 2 * var('GLOBAL_BLOCKS_PER_HOUR', none) * multiplier),
    "worker_batch_size": var(
        (model_name ~ '_' ~ model_type ~ '_worker_batch_size').upper(), 
        (2 * var('GLOBAL_BLOCKS_PER_HOUR', none) * multiplier) // (rpc_config.get('lambdas', 1))
    ),
